...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
        ...
        // check if the end == start
        if (startVertex == null || endVertex == null) {
            throw new NoSuchElementException("Vertex no found");
        }

        HashSet<Vertex> vertexVisited = new HashSet<>();// record vertex visited
        Hashtable<Vertex, Path> distanceTable = new Hashtable<>();// record the current shortest path from start the
                                                                  // other vertex
